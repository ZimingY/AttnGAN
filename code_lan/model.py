    ...
    def init_hidden(self, bsz):
        ...
        if self.rnn_type == 'LSTM':
            return (Variable(weight.new(self.nlayers * self.num_directions,
                                        bsz, self.nhidden).zero_(), requires_grad=True),
...
class NEXT_STAGE_G(nn.Module):
    ...
    def forward(self, h_code, c_code, word_embs, mask):
        """
            h_code1(query):  batch x idf(ngf) x ih x iw (queryL=ihxiw)
            word_embs(context): batch x cdf(emd_size) x sourceL (sourceL=seq_len)
        ...
        # state size ngf/x 2in_size x 2in_size
...
class D_NET256(nn.Module):
    ...
    def forward(self, x_var):
        x_code16 = self.img_code_s16(x_var)     # 16 x 16 x 8df
        x_code8 = self.img_code_s32(x_code16)   # 8 x 8 x 16df
        x_code4 = self.img_code_s64(x_code8)    # 4 x 4 x 32df
        x_code4 = self.img_code_s64_1(x_code4)  # 4 x 4 x 16df
        x_code4 = self.img_code_s64_2(x_code4)  # 4 x 4 x 8df
        return x_code4
